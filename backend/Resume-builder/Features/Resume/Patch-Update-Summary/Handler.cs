...
public class PatchUpdateSummaryHandler(
    AppDbContext db,
    IClaimsService claimsService) : IResponseHandler<PatchUpdateSummaryCommand, ResumeDto>
{
    /// <summary>
    ...
    {
        { nameof(PatchUpdateSummaryRequest.Summary), nameof(ResumeEntity.TextSummary) },
    };

    public async Task<Response<ResumeDto>> Handle(PatchUpdateSummaryCommand command, CancellationToken cancellationToken)
    {
